...
class UserNotesStore:
    def __init__(self, config):
        ...
        self.embeddings = HuggingFaceEmbeddings(model_name=config["user_notes_store"]["embedder_model_name"])
    ...
    def add_documents(self, user_id: str, documents: List[str]):
        chunks = self.text_splitter.split_documents([Document(page_content=doc) for doc in documents])
